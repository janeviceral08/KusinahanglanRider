                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        ...
                        <Card  style={{marginLeft: 15, marginRight:15, paddingBottom: 5, marginBottom: 0, paddingLeft: 5, paddingRight: 5, paddingTop: 5,backgroundColor:'#E8E8E8',borderRadius: 8, flexDirection: 'column' }} key={item.key}>
                            <CardItem button onPress={() => this.props.navigation.navigate('OrderDetails',{ 'orders' : item.datas, 'path' : this.state.store_path, 'RLat' : item.datas.Products[0].slatitude, 'RLong' :  item.datas.Products[0].slongitude, 'RiderId': this.state.uid, 'RiderName': this.state.name, 'RiderToken': this.state.token  })}  >            
